...
class RoomCardEditor extends HTMLElement {
  ...
  // Fordítás betöltése
  _t(key) {
    // Alapértelmezett nyelv detektálása (Config -> HA nyelv -> Angol)
    let lang = this.config.language;
    if (!lang || lang === 'auto') {
        lang = this._hass?.language?.startsWith('hu') ? 'hu' : 'en';
    }
    
    // Visszatérés a kért nyelvvel, vagy fallback angolra/magyarra
  ...
  _szintartomanyFrissit(kulcs, kontainerId, egyseg = '') {
    ...
    tartomanyok.forEach((t, index) => {
      ...
      sor.innerHTML = `
        <input type="number" class="tartomany-tol" placeholder="${this._t('from')}" value="${t.tol ?? ''}" step="any">
        <input type="number" class="tartomany-ig" placeholder="${this._t('to')}" value="${t.ig ?? ''}" step="any">
  ...
  render() {
    ...
    // Nyelv detektálás, és teljes újra-renderelés ha változik a nyelv
    const lang = this.config.language || this._hass?.language || 'hu';
    if (this._currentLang !== lang) {
      this._currentLang = lang;
      this.jartMarItt = false;
    }
    ...
    if (!this.jartMarItt) {
      this.innerHTML = `
        <style>
          ...
          .ful-sor { display: flex; border-bottom: 1px solid var(--divider-color); margin-bottom: 16px; gap: 0; overflow-x: auto; }
          .ful-gomb { flex: 1; background: none; border: none; border-bottom: 2px solid transparent; padding: 10px 4px; font-size: 11px; font-weight: 500; color: var(--secondary-text-color); cursor: pointer; text-transform: uppercase; letter-spacing: 0.05em; transition: color 0.2s, border-color 0.2s; white-space: nowrap; }
          ...
          select.nyelv-kivlaszto { width: 100%; padding: 10px; border-radius: 4px; background: var(--input-fill-color, rgba(128,128,128,0.1)); color: var(--primary-text-color); border: 1px solid var(--divider-color); margin-bottom: 8px;}
          ...
          .hozzaad-gomb { display: flex; align-items: center; gap: 6px; background: none; border: 1px dashed var(--divider-color); border-radius: 8px; padding: 8px 12px; width: 100%; color: var(--secondary-text-color); font-size: 12px; cursor: pointer; margin-top: 4px; box-sizing: border-box; }
          .hozzaad-gomb:hover { border-color: var(--primary-color); color: var(--primary-color); }
          .nyilaszaro-blokk { background: rgba(128,128,128,0.06); border: 1px solid var(--divider-color); border-radius: 8px; padding: 8px; margin-bottom: 8px; display: flex; flex-direction: column; gap: 6px; }
          .nyilaszaro-sor { display: flex; align-items: center; gap: 6px; }
          .nyilaszaro-sor ha-selector { flex: 1; }
          .torles-gomb { background: none; border: none; cursor: pointer; color: var(--error-color); padding: 4px; border-radius: 6px; display: flex; align-items: center; flex-shrink: 0; }
          ...
          .tartomany-sor { display: grid; grid-template-columns: 1fr 1fr 36px 28px; gap: 4px; align-items: center; margin-bottom: 4px; }
          .tartomany-sor input[type="number"] { width: 100%; box-sizing: border-box; background: var(--input-fill-color, rgba(128,128,128,0.1)); border: 1px solid var(--divider-color); border-radius: 6px; padding: 6px 8px; font-size: 12px; color: var(--primary-text-color); }
          .torles-gomb-kis { background: none; border: none; cursor: pointer; color: var(--error-color); padding: 2px; border-radius: 4px; display: flex; align-items: center; }
          .tartomany-fejlec { display: grid; grid-template-columns: 1fr 1fr 36px 28px; gap: 4px; margin-bottom: 2px; }
        ...
        <div class="szerkeszto">
          ...
          <div class="ful-sor">
            <button class="ful-gomb" data-ful="0">${this._t('general')}</button>
            <button class="ful-gomb" data-ful="1">${this._t('sensors')}</button>
            <button class="ful-gomb" data-ful="2">${this._t('doors')}</button>
            <button class="ful-gomb" data-ful="3">${this._t('appearance')}</button>
          </div>
          ...
          <div class="ful-tartalom">
            <div class="mezocsoport">
              <div>
                ...
                <select id="nyelv-selector" class="nyelv-kivlaszto">
              ...
              <div>
                ...
                <ha-textfield id="nev" placeholder="${this._t('room_name_ph')}"></ha-textfield>
              </div>
            </div>
          </div>
          ...
          <div class="ful-tartalom">
            <div class="mezocsoport">
              <div class="csoport-cim">${this._t('temp')}</div>
              <div>
            ...
            <div class="mezocsoport">
              <div class="csoport-cim">${this._t('humidity')}</div>
              <div>
              ...
              <div>
                <div class="al-label">${this._t('absolute')}</div>
                <ha-selector id="abs-paratartalom-entitas"></ha-selector>
              </div>
            </div>

            <div class="mezocsoport">
              <div class="csoport-cim">${this._t('air_quality')}</div>
              <div>
          ...
          <div class="ful-tartalom">
            <div class="mezocsoport">
              <div class="csoport-cim">${this._t('colors')}</div>
              <div>
            ...
            <div class="mezocsoport">
              <div class="csoport-cim">${this._t('shape')}</div>
              <div>
            ...
            <div class="mezocsoport">
              <div class="csoport-cim">${this._t('font_sizes')}</div>
              <div>
                <label>${this._t('room_name')}</label>
                <div class="slider-sor">
      ...
      this.querySelector('#nyelv-selector')?.addEventListener('change', e => {
        this._valueChanged('language', e.target.value);
      });

      this.querySelector('#nev')?.addEventListener('change', e => this._valueChanged('nev', e.target.value));
      this.querySelector('#szin-picker')?.addEventListener('input', e => this._valueChanged('textColor', e.target.value));
      this.querySelector('#kartya-hatter-picker')?.addEventListener('input', e => this._valueChanged('cardBackgroundColor', e.target.value));
      this.querySelector('#kartya-hatter-reset')?.addEventListener('click', () => {
      ...
      this.querySelector('#homerseklet-entitas')?.addEventListener('value-changed', e => this._valueChanged('homerseklet_entity', e.detail.value));
      this.querySelector('#kulso-homerseklet-entitas')?.addEventListener('value-changed', e => this._valueChanged('kulso_homerseklet_entity', e.detail.value));
      this.querySelector('#paratartalom-entitas')?.addEventListener('value-changed', e => this._valueChanged('paratartalom_entity', e.detail.value));
      this.querySelector('#abs-paratartalom-entitas')?.addEventListener('value-changed', e => this._valueChanged('abs_paratartalom_entity', e.detail.value));
      this.querySelector('#szallópor-entitas')?.addEventListener('value-changed', e => this._valueChanged('szallópor_entity', e.detail.value));
      this.querySelector('#levegominoseg-entitas')?.addEventListener('value-changed', e => this._valueChanged('levegominoseg_entity', e.detail.value));

      this.querySelector('#homerseklet-tartomany-hozzaad')?.addEventListener('click', () => this._szintartomanyHozzaad('homerseklet_tartomanyok', 'homerseklet-tartomanyok'));
      this.querySelector('#paratartalom-tartomany-hozzaad')?.addEventListener('click', () => this._szintartomanyHozzaad('paratartalom_tartomanyok', 'paratartalom-tartomanyok'));
      this.querySelector('#szallópor-tartomany-hozzaad')?.addEventListener('click', () => this._szintartomanyHozzaad('szallópor_tartomanyok', 'szallópor-tartomanyok'));
      this.querySelector('#levego-tartomany-hozzaad')?.addEventListener('click', () => this._szintartomanyHozzaad('levego_tartomanyok', 'levego-tartomanyok'));

      this.querySelector('#nyilaszaro-hozzaad')?.addEventListener('click', () => {
        const ujEntitasok = [...(this.config.nyilaszaro_entitasok ?? []), { entity: '', nev: '' }];
        this._valueChanged('nyilaszaro_entitasok', ujEntitasok);
        this._nyilaszaroListaFrissit();
      });

      this.querySelector('#sarok-lekerekites')?.addEventListener('input', e => {
        const val = parseInt(e.target.value);
        this.querySelector('#sarok-lekerekites-ertek').textContent = val + 'px';
        this._valueChanged('sarokLekerekites', val);
      });
      this.querySelector('#nev-betumeret')?.addEventListener('input', e => {
        const val = parseInt(e.target.value);
        this.querySelector('#nev-betumeret-ertek').textContent = val + 'px';
        this._valueChanged('nevBetumeret', val);
      });
      this.querySelector('#homerseklet-betumeret')?.addEventListener('input', e => {
        const val = parseInt(e.target.value);
        this.querySelector('#homerseklet-betumeret-ertek').textContent = val + 'px';
        this._valueChanged('homersekletBetumeret', val);
      });
      this.querySelector('#chip-betumeret')?.addEventListener('input', e => {
        const val = parseInt(e.target.value);
        this.querySelector('#chip-betumeret-ertek').textContent = val + 'px';
        this._valueChanged('chipBetumeret', val);
      });
      this.querySelector('#chip-ikonmeret')?.addEventListener('input', e => {
        const val = parseInt(e.target.value);
...
class RoomCard extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `
      ...
      <style>
        ha-card { background: ${cardBackgroundColor}; border: 1px solid rgba(255,255,255,0.07); border-radius: ${sarokLekerekites}px; overflow: hidden; }
        .kartya-belso { display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; gap: 8px; }
        ...
        .chip { display: inline-flex; align-items: center; gap: 3px; font-size: ${chipBetumeret}px; color: ${textColor}; background: rgba(128,128,128,0.12); border-radius: 6px; padding: 2px 6px; }
